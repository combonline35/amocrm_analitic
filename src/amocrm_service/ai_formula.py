...
- ОТНОСИТЕЛЬНЫЙ период (пользователь сказал «текущий месяц», «этот месяц», «в этом месяце», «за месяц», «прошлый месяц», «эта неделя», «прошлая неделя», «сегодня», «последние N дней») -> используй ПРЕСЕТЫ: op="this_month"/"previous_month"/"this_week"/"previous_week"/"last_days" с value=null. НЕ конвертируй их в конкретные даты — показатель на дашборде должен пересчитываться каждый месяц сам.
- АБСОЛЮТНЫЙ период (пользователь назвал конкретный месяц/год/диапазон: «июль 2026», «за 2025 год», «с 1 по 15 марта») -> используй date_between с конкретными датами или created_month op="eq". Такой период должен остаться зафиксированным навсегда.
- Строка «Сегодня <дата>» в начале контекста нужна ТОЛЬКО чтобы понять, какой сейчас год для запросов вроде «за июнь» (месяц без года). НЕ используй её, чтобы превращать «текущий месяц» в конкретную дату.
- Примеры: «сделки за текущий месяц» -> where: [{"field":"created_at","op":"this_month","value":null}] (плавающий). «сделки за июль 2026» -> where: [{"field":"created_at","op":"date_between","value":["2026-07-01","2026-07-31"]}] (фиксированный).
...
- Выбор view по смыслу запроса:
  view="number" — ответ ОДНО число: «сколько сделок за месяц», «средний чек», «конверсия в процентах».
  view="bar" — рейтинг/сравнение ОДНОЙ метрики по группам, когда групп немного (до ~12): «сделки по менеджерам», «топ источников», «конверсия по этапам». Горизонтальные полосы читаются лучше таблицы.
  view="line" — ДИНАМИКА во времени: «по месяцам», «динамика заявок», «как менялось». Формула должна группировать по месяцу (group_by="created_month" или cf_month_<id>).
  view="list" — топ-N с названиями, когда важен порядок, а не сравнение величин: «топ-5 источников».
  view="table" — НЕСКОЛЬКО показателей на группу (op="table" с columns) или пользователь явно просит таблицу.
- Приоритет выбора: одна метрика + группы -> bar; одна метрика + месяцы -> line; несколько метрик на группу -> table; одно число без группировки -> number.
...
def _simple_count_draft(
    *,
    ...
    # Упоминание конкретного менеджера/ответственного ("у менеджера Сырцов",
    # "по ответственному Иванову", "чьи сделки") — это фильтр по человеку,
    # заготовка его не умеет и уступает модели (у которой есть справочник
    # users). Разрезы "по менеджерам"/"по ответственным" (wants_responsible_group)
    # остаются заготовке — это группировка, а не фильтр по имени.
    mentions_manager_person = not wants_responsible_group and bool(
        tokens & {
            "менеджер", "менеджера", "менеджеру", "менеджером", "менеджеров",
            "ответственный", "ответственного", "ответственному", "ответственном",
            "сотрудник", "сотрудника", "сотруднику", "сотрудником", "сотрудников",
            "кого", "чьи", "чья", "чей",
        }
    )
    if has_field_condition or mentions_manager_person:
        return None
